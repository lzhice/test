    ...
    Rectangle {
        ...
        Rectangle{
            ...
            ListView {
                ...
                onWidthChanged: {
                    console.log("VideoRangSliderItem ListView onWidthChanged:--------------------------------------",width)
                    var count=width/((sliderHeight-10)*1.68)
                    model=count
                    eventManager.sendToWidgetStart("videoThumbUpdate");
                    eventManager.addValue("videoThumbUpdate",count);
                    eventManager.addValue("videoThumbUpdate",(sliderHeight-10)*1.68);
                    eventManager.sendToWidgetEnd("videoThumbUpdate");
                }
                delegate:Image {
                    source: QtImages.load("videoThumb_"+index, "");
                    width: (sliderHeight-10)*1.68
        ...
        onEmitQmlEvent:
        {
            if("VideoRangSliderItem"===eventName){
                ...
                if(value["event"]==="setMaxValue"){
                    ...
                    var tmpWidth=videoThumbList.width
                    videoThumbList.width+=0.01
                    videoThumbList.width=tmpWidth
                    setStartValue(0)
                ...
                }else if(value==="updateVideoThumb"){
                    var tmpCount=videoThumbList.model
                    videoThumbList.model=0;
                    videoThumbList.model=tmpCount;
                }
